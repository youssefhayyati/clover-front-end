...
const validate = () => {
  ...
  errors.value = { name: "", email: "", message: "", phone: "" };
...
const submit = async () => {
  ...
  const { error, data } = await useFetch(
    `${useRuntimeConfig().public.backendUrl}contacts`,
    {
      method: "POST",
      body: contact,
    }
  ...
  if (error && data.length === 0) {
    $nt.show(() =>
      h("div", { class: "text-red-400" }, "Failed to send the message.")
    );
    return;
  }

  $nt.show(() => h("div", { class: "text-green-400" }, "Message sent!"));
};
</script>
...
<template>
  <div class="max-w-6xl max-lg:max-w-3xl mx-auto p-4">
    ...
    >
      <h2 class="text-3xl text-gray-800 font-extrabold text-center mb-12">
        Contact us
      </h2>

      <div class="grid lg:grid-cols-2 items-start gap-12">
        <form @submit.prevent="submit" class="space-y-3 text-gray-800">
          <div>
            <input
              v-model="name"
              type="text"
              placeholder="Name"
              :class="[
                'w-full h-9 px-4 py-1 text-sm border rounded-md focus:ring-2 focus:outline-none',
                errors.name
          ...
          <div>
            <textarea
              ...
              rows="6"
              :class="[
                'w-full px-4 py-1 text-sm border rounded-md focus:ring-2 focus:outline-none',
                errors.message
                  ? 'border-red-400 focus:ring-red-300'
                  : 'border-green-200 focus:ring-green-300',
              ]"
          ...
          <button
            type="submit"
            class="text-white bg-green-400 hover:bg-green-500 rounded text-sm px-6 py-3 !mt-6"
          >
            Send message
          </button>
        </form>
